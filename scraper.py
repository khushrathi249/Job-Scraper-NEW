...
# ... (other helper functions like extract_experience_from_description, extract_detailed_location can be kept here) ...
...
def run_linkedin_scraper(queries, limit=50):
    ...
    # Manually limit the number of queries to avoid excessive scraping time
    for query in queries[:5]: # Limit to first 5 generated queries for speed
        if linkedin_scraped_count >= limit:
            break
        scraper.run([query])
...
# --- IIMJOBS SCRAPER ---
# ... (scrape_iimjobs_page and run_iimjobs_scraper functions remain unchanged) ...
def scrape_iimjobs_page(url, scroll_times=2):
    jobs = []
    with sync_playwright() as p:
        try:
...
def run_full_scrape(linkedin_limit, iimjobs_limit):
    ...
    queries = [Query(query="startup", options=QueryOptions(limit=linkedin_limit, filters=QueryFilters(time=TimeFilters.WEEK)))]
    df_linkedin = run_linkedin_scraper(queries, limit=linkedin_limit)
    df_iim = run_iimjobs_scraper(scroll_limit=iimjobs_limit)
    if df_linkedin.empty and df_iim.empty: return pd.DataFrame()
    return pd.concat([df_linkedin, df_iim], ignore_index=True).fillna('').drop_duplicates(subset=['Company', 'Role', 'Location'], keep='first')

# --- NEW FUNCTION FOR TARGETED SCRAPING ---
def scrape_targeted_jobs(role, location, limit=25):
    """Runs a targeted LinkedIn scrape based on user search terms."""
    if not role and not location:
        return pd.DataFrame()
    
    search_query = f"{role} startup"
    locations_list = [location] if location else None # Scraper expects a list or None
    ...
    df_linkedin = run_linkedin_scraper(queries, limit=limit)
